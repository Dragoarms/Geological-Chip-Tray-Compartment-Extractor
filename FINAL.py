...
Author: GSY - (using Claude Sonnet 3.7)
"""
...
class TesseractManager:
    ...
    def extract_metadata_patterns(self, ocr_text: str) -> Dict[str, Any]:
        ...
        result = {
            ...
            'depth_from': None,
            'depth_to': None,
            'confidence': 0.0  # Adding confidence field to track uncertainty
        }
        ...
        for line in lines:
            ...
            # Match any two uppercase letters followed by 4 digits
            hole_id_match = re.search(r'([A-Z]{2}\s*[0-9OD]{4})', line, re.IGNORECASE)
            if hole_id_match:
                # Extract raw text and clean up spaces
                raw_id = re.sub(r'\s+', '', hole_id_match.group(0)).upper()
                
                # Validate and correct the hole ID
                valid_id = self.validate_hole_id(raw_id)
                if valid_id:
                    result['hole_id'] = valid_id
                    result['confidence'] += 40.0
                    logger.info(f"Found valid hole ID: {valid_id}")
                    continue
            ...
            if depth_match:
                try:
                    # Convert to integers for whole number validation
                    depth_from = int(depth_match.group(1))
                    depth_to = int(depth_match.group(2))
                    
                    # Check if depths already have a 20-unit difference
                    if depth_to - depth_from == 20:
                        result['depth_from'] = float(depth_from)
                        result['depth_to'] = float(depth_to)
                        result['confidence'] += 60.0  # High confidence for perfect match
                        logger.info(f"Found valid depth range with 20-unit difference: {depth_from}-{depth_to}")
                    else:
                        # Check if either depth is divisible by 20
                        from_divisible = depth_from % 20 == 0
                        to_divisible = depth_to % 20 == 0
                        
                        if from_divisible and not to_divisible:
                            # Use depth_from as reference
                            result['depth_from'] = float(depth_from)
                            result['depth_to'] = float(depth_from + 20)
                            result['confidence'] += 40.0  # Moderate confidence
                            logger.info(f"Using divisible depth_from ({depth_from}) to calculate depth_to: {result['depth_to']}")
                        elif to_divisible and not from_divisible:
                            # Use depth_to as reference
                            result['depth_to'] = float(depth_to)
                            result['depth_from'] = float(depth_to - 20)
                            result['confidence'] += 40.0  # Moderate confidence
                            logger.info(f"Using divisible depth_to ({depth_to}) to calculate depth_from: {result['depth_from']}")
                        else:
                            # Neither or both are divisible by 20, flag for user validation
                            result['depth_from'] = float(depth_from)
                            result['depth_to'] = float(depth_to)
                            # Lower confidence to trigger user validation
                            result['confidence'] += 20.0
                            logger.warning(f"Uncertain depth values, will prompt user: {depth_from}-{depth_to}")
                except ValueError:
                    logger.warning(f"Found depth-like pattern but couldn't convert to integers: {depth_match.group(0)}")
        
        # If we still don't have results, try more aggressive patterns
        if result['hole_id'] is None:
            # Try to find any pattern that looks like two uppercase letters followed by 4 digits
            all_text = ' '.join(lines)
            hole_id_match = re.search(r'([A-Z]{2}\s*\d{4})', all_text, re.IGNORECASE)
            if hole_id_match:
                result['hole_id'] = re.sub(r'\s+', '', hole_id_match.group(0)).upper()
                result['confidence'] += 25.0  # Lower confidence for fallback method
                logger.info(f"Found hole ID (fallback method): {result['hole_id']}")
        
        # If we have one depth but not both, calculate the missing one
        if (result['depth_from'] is not None and result['depth_to'] is None):
            result['depth_to'] = result['depth_from'] + 20.0
            result['confidence'] += 30.0  # Moderate confidence for calculated value
            logger.info(f"Calculated depth_to from depth_from: {result['depth_to']}")
        elif (result['depth_from'] is None and result['depth_to'] is not None):
            result['depth_from'] = result['depth_to'] - 20.0
            result['confidence'] += 30.0  # Moderate confidence for calculated value
            logger.info(f"Calculated depth_from from depth_to: {result['depth_from']}")
        
        # Final confidence score that will trigger validation for uncertain results
        if result['hole_id'] is None or result['depth_from'] is None or result['depth_to'] is None:
            # Missing required fields, ensure user validation
            result['confidence'] = min(result['confidence'], 50.0)
        else:
    ...
    def extract_metadata_with_composite(self, 
        ...
        try:
            ...
            # Log final results
            logger.info("------------------------------------------------")
            logger.info("Composite OCR results:")
            if final_hole_id:
    ...
    def _preprocess_for_ocr(self, input_image: np.ndarray, original_filename: Optional[str] = None) -> np.ndarray:
        """
        Adaptive image preprocessing for OCR with progressive testing.
        Applies preprocessing steps incrementally and tests OCR quality at each stage.
        ...
        Returns:
            np.ndarray: Best preprocessed image for OCR
        """
        try:
            ...
            # Store intermediate results with OCR confidence scores
            preprocessing_results = []
            debug_images = {"original_gray": gray}
            
            # Test OCR on original grayscale image
            original_score, original_text = self._test_ocr_quality(gray)
            ...
            logger.info(f"Original grayscale image OCR confidence: {original_score:.2f}%")
            
            # 1. Border removal preprocessing
            border_removed = self._remove_borders(gray)
            if border_removed is not None:
                debug_images["border_removed"] = border_removed
                border_score, border_text = self._test_ocr_quality(border_removed)
                preprocessing_results.append({
                    "image": border_removed,
                    "name": "border_removed",
                    "confidence": border_score,
                    "text": border_text
                })
                logger.info(f"Border removed image OCR confidence: {border_score:.2f}%")
            
            # 2. Noise reduction with bilateral filter
            filtered = cv2.bilateralFilter(gray, 11, 17, 17)
            debug_images["filtered"] = filtered
            filter_score, filter_text = self._test_ocr_quality(filtered)
            preprocessing_results.append({
                "image": filtered,
                "name": "filtered",
            ...
            logger.info(f"Filtered image OCR confidence: {filter_score:.2f}%")
            ...
            binary_score, binary_text = self._test_ocr_quality(binary)
            preprocessing_results.append({
                "image": binary,
                "name": "binary",
            ...
            logger.info(f"Binary threshold image OCR confidence: {binary_score:.2f}%")
            
            # 4. Inversion test - check if inverted image improves OCR
            inverted = cv2.bitwise_not(binary)
            debug_images["inverted"] = inverted
            invert_score, invert_text = self._test_ocr_quality(inverted)
            preprocessing_results.append({
                "image": inverted,
                "name": "inverted",
                "confidence": invert_score,
                "text": invert_text
            })
            logger.info(f"Inverted image OCR confidence: {invert_score:.2f}%")
            ...
            open_score, open_text = self._test_ocr_quality(opened)
            preprocessing_results.append({
                "image": opened,
                "name": "opened",
            ...
            logger.info(f"Opened image OCR confidence: {open_score:.2f}%")
            
            # Closing to fill small holes in characters
            closed = cv2.morphologyEx(opened, cv2.MORPH_CLOSE, kernel)
            debug_images["closed"] = closed
            close_score, close_text = self._test_ocr_quality(closed)
            preprocessing_results.append({
                "image": closed,
                "name": "closed",
            ...
            logger.info(f"Closed image OCR confidence: {close_score:.2f}%")
            
            # Find the best preprocessing result based on OCR confidence
            best_result = max(preprocessing_results, key=lambda x: x["confidence"])
            logger.info(f"Best preprocessing method: {best_result['name']} with {best_result['confidence']:.2f}% confidence")
            logger.info(f"Extracted text: '{best_result['text']}'")
            
            # Save debug images if enabled
            if hasattr(self, 'config') and self.config.get('save_debug_images', False) and original_filename:
                # Check if we have access to a FileManager instance
                file_manager = getattr(self, 'file_manager', None)
                if file_manager is not None and hasattr(self, 'metadata') and self.metadata.get('hole_id'):
                    # Use FileManager to save debug images
                    hole_id = self.metadata.get('hole_id')
                    depth_from = self.metadata.get('depth_from', 0)
                    depth_to = self.metadata.get('depth_to', 0)
                    
                    # Save each debug image
                    for name, img in debug_images.items():
                        file_manager.save_debug_image(
                            img, 
                            hole_id, 
                            depth_from, 
                            depth_to, 
                    ...
                    file_manager.save_debug_image(
                        ...
                        depth_from,
                        depth_to,
                        f'ocr_BEST_{best_result["name"]}_{best_result["confidence"]:.0f}pct'
                    )
                else:
                    # Only save a minimal set of debug images to a temporary location
                    # This significantly reduces the number of debug images saved
                    if file_manager is not None:
                        # Save only the best result image to a temporary location
                        file_manager.save_temp_debug_image(
                            best_result["image"], 
                            original_filename, 
                            f'ocr_best_{best_result["name"]}'
                        )
                    else:
                        # Fallback to basic directory creation if no file manager
                        debug_dir = os.path.join(os.path.dirname(os.path.abspath(original_filename)), 
                                            self.config.get('debug_folder', 'debug'))
                        os.makedirs(debug_dir, exist_ok=True)
                        
                        # Save only the best result image
                        debug_path = os.path.join(debug_dir, generate_debug_filename(
                            original_filename, f'ocr_BEST_{best_result["name"]}'))
                        cv2.imwrite(debug_path, best_result["image"])
    ...
    def _test_ocr_quality(self, image: np.ndarray) -> Tuple[float, str]:
        """
        Test OCR quality on an image and return confidence score and extracted text.
        
        Args:
            image: Preprocessed image to test
        
        Returns:
            Tuple of (confidence_score, extracted_text)
        """
        try:
            ...
            # Configure OCR for testing quality
            config = "--psm 6 --oem 3"  # Single uniform block of text
            
            # Get data with confidence information
            ...
            if confidences:
                ...
                return max(0.0, min(avg_confidence, 100.0)), text
            
            return 0.0, text
            
        except Exception as e:
            logger.error(f"Error testing OCR quality: {str(e)}")
            return 0.0, ""
    ...
    def extract_metadata_from_image(
        ...
        """
        Extract metadata from image using a simplified approach focused directly on the text.
        ...
        try:
            ...
            if 24 in markers:
                ...
                # Create a metadata region by combining both regions
                # This is important for visualization and debugging
                metadata_region = image[hole_id_region_y1:depth_region_y2, region_x1:region_x2].copy()
            else:
                # Fallback: use the bottom portion of the image
                logger.warning("Marker 24 not found, using fallback region detection")
            ...
            # Save debug images - make sure to include these in visualization
            if original_filename and hasattr(self, 'config') and self.config.get('save_debug_images', False):
                # Use the FileManager if available
                if hasattr(self, 'file_manager') and self.file_manager is not None:
                    # Check if we have metadata already
                    hole_id = getattr(self, 'metadata', {}).get('hole_id')
                    depth_from = getattr(self, 'metadata', {}).get('depth_from')
                    depth_to = getattr(self, 'metadata', {}).get('depth_to')
                    
                    if hole_id and depth_from is not None and depth_to is not None:
                        # Save with proper metadata
                        self.file_manager.save_debug_image(viz_image, hole_id, depth_from, depth_to, 'ocr_input')
                        self.file_manager.save_debug_image(hole_id_region, hole_id, depth_from, depth_to, 'hole_id_region')
                        self.file_manager.save_debug_image(depth_region, hole_id, depth_from, depth_to, 'depth_region')
                        self.file_manager.save_debug_image(viz_image, hole_id, depth_from, depth_to, 'metadata_regions_viz')
                    else:
                        # Use temporary storage until metadata is available
                        self.file_manager.save_temp_debug_image(viz_image, original_filename, 'ocr_input')
                        self.file_manager.save_temp_debug_image(hole_id_region, original_filename, 'hole_id_region')
                        self.file_manager.save_temp_debug_image(depth_region, original_filename, 'depth_region')
                        self.file_manager.save_temp_debug_image(viz_image, original_filename, 'metadata_regions_viz')
                else:
                    # Log warning about missing FileManager
                    logger.warning(f"FileManager not available for saving debug images")
            ...
            result = {
                ...
                'confidence': 0.0,
                'metadata_region': metadata_region,
                'metadata_region_viz': viz_image,  # Important for visualization
                'ocr_text': ""
            }
            
            # Process hole ID region - using a much simpler approach focused on clarity
            if progress_queue is not None:
                progress_queue.put(("Extracting hole ID...", None))
            
            # For each region, try multiple preprocessing techniques
            hole_id = self._extract_text_from_region(hole_id_region, 
                                                is_hole_id=True, 
                                                original_filename=original_filename,
                                                debug_prefix="hole_id")
            ...
            # Process depth region
            if progress_queue is not None:
                progress_queue.put(("Extracting depth range...", None))
            
            depth_text = self._extract_text_from_region(depth_region, 
                                                    is_hole_id=False,
                                                    original_filename=original_filename,
                                                    debug_prefix="depth")
            
            # Parse depth range from text
            if depth_text:
                # Remove all non-digit and non-dash characters
                clean_text = re.sub(r'[^0-9\-]', '', depth_text)
                
                # Try to match digit-dash-digit pattern
            ...
            result['ocr_text'] = f"Hole ID: {hole_id or ''}\nDepth: {depth_text or ''}"
            
            # Final validation
            # If we are missing either value, set confidence lower to trigger user validation
            if result['hole_id'] is None or result['depth_from'] is None or result['depth_to'] is None:
                result['confidence'] = min(result['confidence'], 50.0)
            ...
            logger.info(f"Hole ID: {result['hole_id']}")
            logger.info(f"Depth range: {result['depth_from']} - {result['depth_to']}")
    ...
    def _extract_text_from_region(self, region: np.ndarray, is_hole_id: bool = False, 
                                original_filename: Optional[str] = None,
                                debug_prefix: str = "region") -> Optional[str]:
        """
        Extract text from an image region using multiple preprocessing techniques optimized
        for high-contrast labels.
        
        Args:
            region: Image region to process
            is_hole_id: Whether this region contains a hole ID (affects processing)
            original_filename: Optional original filename for debug images
        ...
        if region is None or region.size == 0:
            logger.warning("Empty region provided for text extraction")
            return None
        ...
        # Convert to grayscale if needed
        if len(region_copy.shape) == 3:
            gray = cv2.cvtColor(region_copy, cv2.COLOR_BGR2GRAY)
        else:
            gray = region_copy.copy()
        
        # Save debug images
        debug_images = {}
        debug_images["original"] = region_copy
        debug_images["gray"] = gray
        
        # Store all extraction attempts
        extraction_results = []
        
        # 1. Basic Otsu threshold (best for clean black text on white)
        _, otsu_bin = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
        debug_images["otsu_bin"] = otsu_bin
        
        # Inverted version (best for white text on black)
        otsu_bin_inv = cv2.bitwise_not(otsu_bin)
        debug_images["otsu_bin_inv"] = otsu_bin_inv
        ...
        debug_images["adaptive_bin"] = adaptive_bin
        
        # Inverted version
        adaptive_bin_inv = cv2.bitwise_not(adaptive_bin)
        debug_images["adaptive_bin_inv"] = adaptive_bin_inv
        
        # 3. Fixed threshold (good for consistent contrast)
        _, fixed_bin = cv2.threshold(gray, 127, 255, cv2.THRESH_BINARY)
        debug_images["fixed_bin"] = fixed_bin
        
        # Inverted version
        fixed_bin_inv = cv2.bitwise_not(fixed_bin)
        debug_images["fixed_bin_inv"] = fixed_bin_inv
        
        # 4. Try edge enhancement
        ...
        # List of images to try for OCR
        images_to_try = [
            ("otsu_bin", otsu_bin),
            ("otsu_bin_inv", otsu_bin_inv),
            ("adaptive_bin", adaptive_bin),
            ("adaptive_bin_inv", adaptive_bin_inv),
            ("fixed_bin", fixed_bin),
            ("fixed_bin_inv", fixed_bin_inv),
            ("gray", gray)
        ]
        
        # Different PSM modes to try (page segmentation modes)
        psm_modes = [8, 7, 6, 10, 3] if is_hole_id else [7, 8, 6, 3]
        
        # Configure character whitelist based on content type
        if is_hole_id:
            # For hole IDs: letters + numbers (with common OCR confusions)
            char_whitelist = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789ODl"
        else:
            # For depths: mainly numbers and separators
            char_whitelist = "0123456789-."
        
        # Try different combinations of preprocessing and OCR settings
        for img_name, img in images_to_try:
            for psm in psm_modes:
                try:
                    # Configure OCR
                    config = f"--psm {psm} --oem 3 -c tessedit_char_whitelist={char_whitelist}"
                    
                    # Run OCR
                    text = self.pytesseract.image_to_string(img, config=config).strip()
                    
                    # Log result
                    logger.info(f"OCR ({img_name}, PSM {psm}): '{text}'")
                    
                    # Add to results if non-empty
                    if text:
                        extraction_results.append((text, img_name, psm))
                except Exception as e:
                    logger.warning(f"OCR error with {img_name}, PSM {psm}: {str(e)}")
        ...
        if hasattr(self, 'config') and self.config.get('save_debug_images', False) and original_filename:
            if hasattr(self, 'file_manager') and self.file_manager is not None:
                ...
                if hole_id and depth_from is not None and depth_to is not None:
                    # Save with proper metadata
                    for name, img in debug_images.items():
                        self.file_manager.save_debug_image(img, hole_id, depth_from, depth_to, f'{debug_prefix}_{name}')
                else:
                    # Use temporary storage until metadata is available
                    for name, img in debug_images.items():
                        self.file_manager.save_temp_debug_image(img, original_filename, f'{debug_prefix}_{name}')
            else:
                # Log warning about missing FileManager
                logger.warning(f"FileManager not available for saving debug images")
        
        # Process extraction results based on region type
        if is_hole_id:
            # For hole ID: look for pattern XX0000 (two letters followed by four digits)
            for text, img_name, psm in extraction_results:
                # Remove spaces and convert to uppercase
                cleaned = text.replace(' ', '').upper()
                
                # Look for pattern: 2 letters followed by 4 digits/characters
                match = re.search(r'([A-Z]{2})([0-9ODl]{4})', cleaned)
                if match:
                    # Extract and format the hole ID
                    letters = match.group(1)
                    digits = match.group(2)
                    
                    # Replace common OCR mistakes in the numeric part
                    corrected_digits = ""
                    for char in digits:
                        if char in 'ODl':
                            corrected_digits += '0' if char in 'OD' else '1'
                        else:
                            corrected_digits += char
                    
                    # Return the corrected hole ID
                    return letters + corrected_digits
                    
            # If no precise match, try more forgiving pattern
            for text, img_name, psm in extraction_results:
                if len(text) >= 6:
                    # Try to extract anything that looks like a hole ID
                    cleaned = text.replace(' ', '').upper()
                    
                    # Look for 2 letters anywhere
                    letter_match = re.search(r'([A-Z]{2})', cleaned)
                    if letter_match:
                        letters = letter_match.group(1)
                        # Look for 4 digits after the letters
                        remaining = cleaned[cleaned.index(letters) + 2:]
                        digit_match = re.search(r'([0-9ODl]{4})', remaining)
                        if digit_match:
            ...
            # For depth range: look for pattern like "000-020"
            for text, img_name, psm in extraction_results:
                # Remove all non-digit and non-dash characters
                cleaned = re.sub(r'[^0-9\-]', '', text)
                
                # Look for digit-dash-digit pattern
                match = re.search(r'(\d+)-(\d+)', cleaned)
                if match:
                    return f"{match.group(1)}-{match.group(2)}"
                
                # If we have just numbers, try to parse them directly
                digits = re.findall(r'\d+', cleaned)
                if len(digits) >= 2:
                    return f"{digits[0]}-{digits[1]}"
        
        # No valid text found
        return None if not extraction_results else extraction_results[0][0]


    def extract_labels_with_templates(self, metadata_region: np.ndarray, original_filename: Optional[str] = None) -> Dict[str, Any]:
        """
        Extract label text using targeted label detection and OCR with improved preprocessing.
        """
        try:
            # Convert to grayscale if needed
            ...
            # Enhance contrast to make labels more visible
            clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8, 8))
            enhanced = clahe.apply(gray)
            debug_images["enhanced"] = enhanced
            
            # Try multiple thresholding approaches to improve label detection
            label_regions = []
            thresholding_methods = [
                ...
                # Fixed threshold with moderately low value to catch darker borders
                ("fixed_low", lambda img: cv2.threshold(img, 80, 255, cv2.THRESH_BINARY_INV)[1]),
                # Fixed threshold with higher value for stronger contrast
                ("fixed_high", lambda img: cv2.threshold(img, 120, 255, cv2.THRESH_BINARY_INV)[1])
            ]
            
            # Process each thresholding method
            for method_name, threshold_func in thresholding_methods:
                thresh = threshold_func(enhanced)
                debug_images[f"thresh_{method_name}"] = thresh
                
                # Clean up with morphological operations
                kernel = np.ones((3, 3), np.uint8)
                dilated = cv2.dilate(thresh, kernel, iterations=1)
                debug_images[f"dilated_{method_name}"] = dilated
                
                # Find contours
                contours, _ = cv2.findContours(dilated.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
                
                # Filter contours to find rectangular label shapes
                method_regions = []
                for contour in contours:
                    # Get bounding rectangle
                    x, y, w, h = cv2.boundingRect(contour)
                    ...
                    # More relaxed filtering criteria for label rectangles
                    # Labels are usually wider than tall (aspect ratio > 1)
                    if 1.5 < aspect_ratio < 10.0 and area > 300:
                        rect_area = w * h
                        contour_area = cv2.contourArea(contour)
                        fill_ratio = contour_area / rect_area
                        
                        # Labels typically have a high fill ratio (mostly filled rectangles)
                        if fill_ratio > 0.6:
                            method_regions.append((x, y, w, h))
                
                # Add regions from this method
                label_regions.extend(method_regions)
                
                # Create visualization of detected regions for this method
                method_viz = metadata_region.copy()
                for i, (x, y, w, h) in enumerate(method_regions):
                    cv2.rectangle(method_viz, (x, y), (x+w, y+h), (0, 255, 0), 2)
                    cv2.putText(method_viz, f"{method_name} {i+1}", (x, y-5), 
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
                debug_images[f"regions_{method_name}"] = method_viz
            ...
            filtered_regions.sort(key=lambda r: r[1])
            ...
            # Process each detected label individually
            # Typically, we expect the first label to be the hole ID and the second to be the depth range
            for i, (x, y, w, h) in enumerate(label_regions[:2]):  # Process at most 2 labels
                # Extract the label region, adding a small margin to avoid cutting off characters
                margin = 2
                x1 = max(0, x - margin)
                y1 = max(0, y - margin)
                ...
                label_img = gray[y1:y2, x1:x2]
                
                # Skip very small regions
                if label_img.shape[0] < 10 or label_img.shape[1] < 10:
                    continue
                
                # Remove the black border from the label to improve OCR accuracy
                # Threshold to separate black border from white interior
                _, binary = cv2.threshold(label_img, 100, 255, cv2.THRESH_BINARY)
                
                # Use morphological operations to clean up the binary image
                kernel = np.ones((3, 3), np.uint8)
                cleaned = cv2.morphologyEx(binary, cv2.MORPH_OPEN, kernel)
                
                # Invert to find contours of the white region inside the border
                inverted = cv2.bitwise_not(cleaned)
                
                # Find contours of the white interior region
                inner_contours, _ = cv2.findContours(inverted, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
                
                # Find the largest inner contour, which should be the label content area
                inner_contour = None
                max_area = 0
                ...
                # Create a mask for the inner area to extract just the content
                if inner_contour is not None and max_area > 0:
                    inner_mask = np.zeros_like(label_img)
                    cv2.drawContours(inner_mask, [inner_contour], 0, 255, -1)
                    
                    # Extract just the content using the mask and make it white background
                    content_img = np.ones_like(label_img) * 255
                    content_img[inner_mask == 255] = label_img[inner_mask == 255]
                else:
                    # Fallback to original if no inner contour found
                    content_img = label_img
                
                # Enlarge for better OCR results (4x scale works well for small text)
                resized_label = cv2.resize(content_img, (w*4, h*4), interpolation=cv2.INTER_CUBIC)
                
                # Ensure we have a clean binary image for OCR
                _, label_bin = cv2.threshold(resized_label, 127, 255, cv2.THRESH_BINARY)
                
                # Create inverted version for testing which works better
                label_bin_inv = cv2.bitwise_not(label_bin)
                ...
                # For the first label (usually hole ID)
                if i == 0:
                    # Try both original and inverted binary images
                    for binary_img, binary_name in [(label_bin, "normal"), (label_bin_inv, "inverted")]:
                        # Try different PSM modes optimized for hole ID detection
                        for psm_mode in [8, 7, 6]:  # Try single word first, then line, then block
                            try:
                                # Configure OCR with appropriate options for hole ID
                                if psm_mode == 8:  # Single word mode
                                    config = f"--psm {psm_mode} --oem 3 -c tessedit_char_whitelist=ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789OD"
                                else:
                                    config = f"--psm {psm_mode} --oem 3"
                                    
                                # Perform OCR
                                text = self.pytesseract.image_to_string(binary_img, config=config).strip()
                                logger.info(f"Label {i+1} OCR ({binary_name} PSM {psm_mode}): '{text}'")
                                
                                # Look for hole ID pattern (2 letters followed by 4 digits)
                                # This pattern is more specific to real hole IDs
                                hole_id_match = re.search(r'([A-Z]{2}[0-9OD]{4})', text.upper().replace(" ", ""))
                                
                                if hole_id_match:
                                    # Use the validation function to correct the hole ID
                                    candidate = hole_id_match.group(0)
                                    hole_id = self.validate_hole_id(candidate)
                                    if hole_id:
                                        logger.info(f"Found and validated hole ID: {hole_id}")
                                        break
                                        
                            except Exception as e:
                                logger.warning(f"OCR error for hole ID: {str(e)}")
                        
                        # If we found a hole ID, no need to try other binary versions
                        if hole_id:
                            break
                
                # For the second label (usually depth range)
                elif i == 1:
                    # Try both original and inverted binary images
                    for binary_img, binary_name in [(label_bin, "normal"), (label_bin_inv, "inverted")]:
                        # Try different PSM modes optimized for depth range detection
                        for psm_mode in [7, 8, 6, 3]:  # Line, word, block, and auto page
                            try:
                                # Configure OCR with appropriate options for depth range
                                # Use character whitelist to only detect digits and separators
                                ...
                                # Extract depth range using specialized function
                        ...
                        # If we found depths, no need to try other binary versions
                        if depth_from is not None and depth_to is not None:
                            break
                    
                    # If standard OCR fails, try digit-by-digit detection as fallback
                    if depth_from is None or depth_to is None:
                        logger.info("Trying digit-by-digit detection for depth range")
                        depth_from, depth_to = self._detect_numbers_in_label(binary_img)
                        if depth_from is not None and depth_to is not None:
                            logger.info(f"Digit detection found depths: {depth_from}-{depth_to}")
            ...
            if hole_id:
                if re.match(r'^[A-Z]{2}\d{4}$', hole_id):  # Exact match for standard format
                    confidence += 45.0
                else:
                    confidence += 25.0
            ...
            if depth_from is not None and depth_to is not None:
                # Validate the range is reasonable
                if depth_from < depth_to:
                    # Check if it's a standard 20m interval
                    if abs((depth_to - depth_from) - 20) < 1:
                        confidence += 45.0  # Very high confidence for standard interval
                    else:
                        confidence += 35.0  # Good confidence for valid range
                else:
                    confidence += 15.0  # Low confidence for suspicious range
    ...
    def validate_hole_id(self, hole_id: str) -> Optional[str]:
        """
        Validate and correct a hole ID according to the format rules:
        - Two uppercase letters followed by four digits
        - Convert any 'O' or 'D' in the last 4 positions to '0'
        - Convert any 'l' or 'I' to '1'
        - Check against list of valid prefixes if enabled
        
        Args:
            hole_id: Candidate hole ID string
            
        Returns:
            Corrected hole ID if valid, None otherwise
        """
        if not hole_id or len(hole_id) < 6:
            return None
            
        # Convert to uppercase and remove any spaces
        hole_id = hole_id.upper().replace(" ", "")
        
        # Check if first two characters are letters and there are at least 4 more characters
        if len(hole_id) < 6 or not hole_id[:2].isalpha():
            return None
        
        # Take just the first 6 characters (2 letters + 4 digits)
        hole_id = hole_id[:6]
        
        # Get the prefix (first two letters)
        prefix = hole_id[:2]
        
        # If prefix validation is enabled and we have a config reference
        if hasattr(self, 'config') and self.config.get('enable_prefix_validation', False):
            valid_prefixes = self.config.get('valid_hole_prefixes', [])
            
            # Check if the detected prefix matches any known prefix
            if valid_prefixes and prefix not in valid_prefixes:
                # Try to find the closest valid prefix based on character similarity
                best_match = None
                min_diff = float('inf')
                
                for valid_prefix in valid_prefixes:
                    # Count differing characters
                    diff = sum(1 for a, b in zip(prefix, valid_prefix) if a != b)
                    
                    # Update if this is the best match so far
                    if diff < min_diff:
                        min_diff = diff
                        best_match = valid_prefix
                
                # If we found a close match (only 1 character difference), use it
                if best_match and min_diff <= 1:
                    self.logger.info(f"Corrected hole ID prefix from {prefix} to {best_match}")
                    prefix = best_match
                    # Update the hole_id with the corrected prefix
                    hole_id = prefix + hole_id[2:]
        
        # Convert O/D to 0 and I/l to 1 in the last four positions (numeric part)
        corrected_id = list(hole_id)
        for i in range(2, 6):
            if corrected_id[i] in 'OD':
                corrected_id[i] = '0'
            elif corrected_id[i] in 'Il':
                corrected_id[i] = '1'
        
        corrected_id = ''.join(corrected_id)
        
        # Verify the last four characters are now digits
        if not corrected_id[2:].isdigit():
            return None
...
class DuplicateHandler:
    ...
    def check_duplicate(self, 
        ...
        # NEW: Check Chip Compartment folder for existing compartments
        try:
                        ...
                        if file.endswith(('.png', '.jpg', '.jpeg', '.tif', '.tiff')):
                            ...
                            if match:
                                ...
                                if compartment_depth in compartment_depths:
                                    ...
                                    compartment_files.append(compartment_path)
                                    self.logger.info(f"Found existing compartment at depth {compartment_depth}: {file}")
                
                # If we found existing compartments, mark as duplicate
                if compartment_files:
                    is_duplicate = True
                    self.duplicate_files.extend(compartment_files)
                    self.logger.info(f"Found {len(compartment_files)} existing compartments for {hole_id} between {start_depth}-{end_depth}m")
        except Exception as e:
...
class ChipTrayExtractor:
    ...
    def __init__(self):
        ...
        self.config = {
            ...
            'blurry_threshold_percentage': 20.0,  # Percentage of blurry compartments to flag the tray
    ...
    def detect_blur_in_compartments(self, 
        ...
        Args:
            ...
            save_dir: Optional directory to save blur visualizations (not used - FileManager is used instead)
    ...
    def extract_compartment_boundaries(self, 
        ...
        expected_compartment_ids = self.config['compartment_marker_ids'][:expected_count]
        ...
        try:
            ...
            if self.config['enable_ocr'] and self.tesseract_manager.is_available:
                ...
                try:
                    ...
                    # If the composite method didn't yield good results, try the original method as fallback
                    if not composite_has_data or composite_confidence < self.config['ocr_confidence_threshold']:
                        logger.info(f"Composite OCR confidence too low ({composite_confidence:.1f}%), trying fallback method")
                        if hasattr(self, 'progress_queue'):
                            self.progress_queue.put((f"Trying fallback OCR method...", None))
                        ...
                        fallback_metadata = self.tesseract_manager.extract_metadata_from_image(
                            small_image, markers, original_filename=image_path
                        )
                        
                        # If fallback method got better results, use those instead
                        fallback_confidence = fallback_metadata.get('confidence', 0)
                        fallback_has_data = (fallback_metadata.get('hole_id') is not None and 
                                        fallback_metadata.get('depth_from') is not None and 
                                        fallback_metadata.get('depth_to') is not None)
                        
                        # Compare results and use the better one
                        if fallback_has_data and (not composite_has_data or fallback_confidence > composite_confidence):
                            logger.info(f"Using fallback OCR results (confidence: {fallback_confidence:.1f}%)")
                    ...
                    # Change the needs_confirmation logic to always be True
                    ...
                    if needs_confirmation and self.root is not None:
                        ...
                        if hasattr(self, 'progress_queue'):
                            ...
                            if has_data_issues:
                                dialog_msg = "OCR confidence low, prompting for metadata..."
                        ...
                        # Use OCR metadata without prompting (only if prompt_for_metadata is disabled)
                        metadata = {
                            'hole_id': ocr_metadata.get('hole_id'),
                            'depth_from': ocr_metadata.get('depth_from'),
                            'depth_to': ocr_metadata.get('depth_to')
            ...
            if hasattr(self, 'progress_queue'):
                ...
                num_saved = 0
                if metadata and metadata.get('hole_id'):
                    # Save compartments with proper metadata
                    hole_id = metadata['hole_id']
                    depth_from = metadata.get('depth_from', 0)
                    depth_to = metadata.get('depth_to', 0)
                    
                    # Calculate depth increment per compartment
                    total_depth = float(depth_to) - float(depth_from)
                    depth_increment = total_depth / len(compartments)
                    
                    # Save each compartment with appropriate depth
                    for i, compartment in enumerate(compartments):
                        compartment_depth = int(depth_from + ((i + 1) * depth_increment))
                        
                        try:
                            # Use FileManager to save the compartment
    ...
    def create_gui(self):
        ...
        interval_choices = [0.5, 1.0, 2.0, 5.0]
        ...
        interval_info = ttk.Label(interval_frame, 
                                text="Depth per compartment (e.g., 1m means each compartment spans 1 meter)",
                                font=("Arial", 8),
                                foreground="gray")
        interval_info.pack(side=tk.LEFT, padx=(10, 0))
        ...
        storage_label = ttk.Label(storage_frame, text="Output Location:", width=15, anchor='w')
    ...
    def save_compartments(self, 
        ...
        blur_results = self.detect_blur_in_compartments(compartments, None, base_filename)
        ...
        if metadata and all(key in metadata for key in ['hole_id', 'depth_from', 'depth_to']):
            hole_id = metadata['hole_id']
            ...
            total_depth_range = depth_to - depth_from
            ...
            # Log warning
            logger.warning("Missing metadata for compartment naming, using default values")
